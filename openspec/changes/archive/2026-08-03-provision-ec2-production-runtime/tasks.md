...
- [x] 4.3 Install and enable systemd so Uvicorn runs as `essay-api` on `127.0.0.1:8000` without `--reload`
- [x] 4.4 Install and enable Nginx with SPA fallback, `/api` proxying, forwarded headers, body limits, timeouts, and bounded logs
